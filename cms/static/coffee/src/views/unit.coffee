...
($, ui, gettext, Backbone, NotificationView, PromptView, ModuleModel, ModuleEditView) ->
  class UnitEdit extends Backbone.View
    events:
    ...
    initialize: =>
      @visibilityView = new UnitEdit.Visibility(
        el: @$('.visibility-select')
        model: @model
      )

      @locationView = new UnitEdit.LocationState(
        el: @$('.section-item.editing a')
        model: @model
      )

      @nameView = new UnitEdit.NameEdit(
        el: @$('.unit-name-input')
        model: @model
      )
  ...
  class UnitEdit.NameEdit extends Backbone.View
    events:
  ...
  class UnitEdit.LocationState extends Backbone.View
    initialize: =>
      @model.on('change:state', @render)
  ...
  class UnitEdit.Visibility extends Backbone.View
    initialize: =>
      @model.on('change:state', @render)
      @render()
